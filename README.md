...
Проект создан на базе 3Д моделей для моей игры в жанре "Защити башню". Уровень собирается из квадратов (prefab) с дорогами и стенами (прямые, угловые и т.д.), замком и домиком для спавна врагов. Квадраты с дорогой имеют скрипт TailRoad, за исключением квадрата окончания дороги, который используется для генерации бонусов и имеет скрипт SpawnBonus. В инспекторе добавляем prefab для спавна и его номер (0 - оружие(не спавнится), 1 - монета, 2 - яблоко, 3 - аптечка) чтобы правильно обработать бонус в скрипте PlayerBonus. Квадрат с домиком для спавна врагов имеет скрипт SpawnEnemy. Prefab врага передаётся из BoardMap. У замка 2 скрипта: один для отслеживания получаемого "урона" и второй для управления дверями при попадании героя в соответствующий триггер. 
...
Видео варианта прохождения игрры - https://disk.yandex.ru/i/oAvvcDxPTsWAEw
Билд для Windows - https://disk.yandex.ru/d/_G5uwcWsokgWvg
